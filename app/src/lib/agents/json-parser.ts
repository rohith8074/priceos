export default function parseLLMJson(
  ...
  const {
    ...
    maxDecodeDepth = 3,
    unwrapKeys = [
      'raw_response', 'rawResponse', 'response', 'data', 'result',
      'output', 'content', 'message', 'text', 'completion',
    ],
  ...
  const decodeJsonStringLoop = (value: any, depth = 0): any => {
    ...
    if ((s.startsWith('"') && s.endsWith('"')) || (s.startsWith("'") && s.endsWith("'"))) {
      try { return decodeJsonStringLoop(JSON.parse(s), depth + 1) } catch { return value }
    }
  ...
  const findJsonBoundaries = (t: string): string | null => {
    ...
    let inDq = false, inSq = false, escapeNext = false
    for (let i = 0; i < t.length; i++) {
      ...
      if (!inDq && !inSq) {
        if (ch === '{' || ch === '[') { if (start === -1) start = i; depth++ }
        else if (ch === '}' || ch === ']') { depth--; if (depth === 0 && start !== -1) { end = i + 1; break } }
      }
    }
  ...
  const extractJson = (t: string): string[] => {
    ...
    for (const r of results) {
      if (!seen.has(r.content)) { seen.add(r.content); out.push(r.content) }
  ...
  const tryParseJson = (jsonStr: string) => {
    ...
    try { return { success: true, data: decodeJsonStringLoop(JSON.parse(clean)), error: null } } catch {}
    if (attemptFix) {
      const bounded = findJsonBoundaries(clean)
      if (bounded) { try { return { success: true, data: decodeJsonStringLoop(JSON.parse(bounded)), error: null } } catch {} }
      clean = fixCommonJsonIssues(clean)
      try { return { success: true, data: decodeJsonStringLoop(JSON.parse(clean)), error: null } } catch {}
      const extracted = findJsonBoundaries(clean)
      if (extracted) { try { return { success: true, data: decodeJsonStringLoop(JSON.parse(extracted)), error: null } } catch {} }
    }
  ...
  const unwrapResponse = (data: any) => {
    ...
    for (let depth = 0; depth < maxUnwrapDepth; depth++) {
      ...
      if (preferRawResponse && typeof current?.raw_response === 'string') {
        ...
        if (parsed.success) { current = parsed.data; continue }
      }
      let advanced = false
      for (const k of unwrapKeys) {
        if (current && Object.prototype.hasOwnProperty.call(current, k) && current[k] != null) {
          ...
          if (typeof v === 'string') {
            ...
            if (parsed.success) { current = parsed.data; advanced = true; break }
          } else if (typeof v === 'object') { current = v; advanced = true; break }
        }
      }
      if (!advanced) break
    }
  ...
  return { success: false, data: null, error: 'No valid JSON found in the response', rawJson: null }
}
