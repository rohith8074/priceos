...
import { Sparkles, ArrowLeft, CheckCircle, AlertCircle, Loader2, User, Mail } from "lucide-react";
...
function ForgotPasswordContent() {
  ...
  const handleSendResetLink = async () => {
    ...
    try {
      ...
      const checkRes = await fetch(`/api/auth/admin-reset-password?email=${encodeURIComponent(email)}`);
          ...
          <Card className="bg-white/[0.03] backdrop-blur-3xl border-emerald-500/20 shadow-2xl p-8 space-y-6">
            <div className="flex flex-col items-center text-center space-y-4">
              ...
              <p className="text-sm text-white/50">
                We&apos;ve sent a password reset link to <span className="text-amber-500 font-semibold">{email}</span>.
          ...
          <Card className="bg-white/[0.03] backdrop-blur-3xl border-white/5 shadow-2xl relative overflow-hidden p-0">
            ...
            <div className="p-6 space-y-5">
              ...
              <Button
                ...
                className="w-full h-12 bg-gradient-to-r from-amber-500 to-orange-600 text-black font-black uppercase tracking-wider shadow-lg shadow-amber-500/30 hover:shadow-amber-500/50 hover:-translate-y-0.5 transition-all disabled:opacity-40"
              >
                {isSending ? (
